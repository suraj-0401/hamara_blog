...
import { access_key, contactUrl } from '../Urls';
function Contact() {
  const {
    register,
    handleSubmit,
  ...
  const onSubmit = async(data) => {
    const userInfo={
     ...
     try {
      await axios.post(`${contactUrl}`,userInfo)
  ...
  return (
    <div className="relative flex items-top justify-center min-h-screen bg-white dark:bg-gray-900 sm:items-center sm:pt-0 mt-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2">
            ...
            <form className="p-6 flex flex-col justify-center" onSubmit={handleSubmit(onSubmit)}>
              ...
              <div className="flex flex-col mt-2">
                ...
                resize="none"
                />
                {errors.comments && <span className='text-sm text-red-500 font-semibold'>This field is required</span>}
              </div>

              <button
                type="submit"
                className="w-full mt-2 bg-indigo-500 dark:bg-indigo-700 hover:bg-indigo-700 dark:hover:bg-indigo-500 text-gray-100 font-bold py-3 px-6 rounded-lg"
              >
                Send Message
              </button>
            </form>
